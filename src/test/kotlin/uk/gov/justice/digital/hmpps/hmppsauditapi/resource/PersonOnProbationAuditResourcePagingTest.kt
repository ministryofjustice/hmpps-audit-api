...
class PersonOnProbationAuditResourcePagingTest : IntegrationTest() {

  @Autowired
  private lateinit var personOnProbationAuditRepository: PersonOnProbationAuditRepository

  val probationServiceRequestBody = JSONObject().put("service", "person-on-probation")
  ...
  fun `insert test audit events`() {
    listOfAudits.forEach {
      personOnProbationAuditRepository.save(it)
    }
  }

  @AfterAll
  fun `remove test audit events`() {
    personOnProbationAuditRepository.deleteAll()
  }

  @Test
  fun `find full page of audit events`() {
    webTestClient.post().uri("/audit/persononprobation/paged?page=0&size=4&sort=who")
      .headers(setAuthorisation(roles = listOf("ROLE_PERSON_ON_PROBATION_AUDIT"), scopes = listOf("read")))
      .contentType(MediaType.APPLICATION_JSON)
      .bodyValue(probationServiceRequestBody.toString())
      .exchange()
      .expectStatus().isOk
      .expectBody()
    ...
    verify(auditQueueService).sendAuditAuditEvent(
      "AUDIT_GET_ALL_PAGED",
      AuditFilterDto(startDateTime = null, endDateTime = null, service = "person-on-probation", who = null, what = null),
    )
  }

  @Test
  fun `find full page of descending ordered audit events`() {
    webTestClient.post().uri("/audit/persononprobation/paged?page=0&size=4&sort=who,desc")
      .headers(setAuthorisation(roles = listOf("ROLE_PERSON_ON_PROBATION_AUDIT"), scopes = listOf("read")))
      .contentType(MediaType.APPLICATION_JSON)
      .bodyValue(probationServiceRequestBody.toString())
      .exchange()
  ...
  fun `find first page of audit events`() {
    webTestClient.post().uri("/audit/persononprobation/paged?page=0&size=3&sort=who")
      .headers(setAuthorisation(roles = listOf("ROLE_PERSON_ON_PROBATION_AUDIT"), scopes = listOf("read")))
      .contentType(MediaType.APPLICATION_JSON)
      .bodyValue(probationServiceRequestBody.toString())
      .exchange()
  ...
  fun `find second page of audit events`() {
    webTestClient.post().uri("/audit/persononprobation/paged?page=2&size=1&sort=who")
      .headers(setAuthorisation(roles = listOf("ROLE_PERSON_ON_PROBATION_AUDIT"), scopes = listOf("read")))
      .contentType(MediaType.APPLICATION_JSON)
      .bodyValue(probationServiceRequestBody.toString())
      .exchange()
