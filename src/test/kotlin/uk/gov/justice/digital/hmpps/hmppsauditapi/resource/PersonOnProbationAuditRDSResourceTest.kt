...
@Suppress("ClassName")
class PersonOnProbationAuditRDSResourceTest : IntegrationTest() {

  @TestInstance(PER_CLASS)
  @Nested
  inner class SecureGetEndpoints {
    private fun secureEndpointsGet() = listOf(
      "/audit/persononprobation",
    )
  ...
  inner class securePostEndpoints {
    ...
    internal fun `satisfies the correct role but no scope`(uri: String) {
      webTestClient.post()
        .uri(uri)
        .headers(setAuthorisation(roles = listOf("ROLE_PERSON_ON_PROBATION_AUDIT")))
        .body(BodyInserters.fromValue(AuditEvent(what = "what")))
        .exchange()
        .expectStatus().isForbidden
    }
    ...
    internal fun `satisfies the correct role and scope`(uri: String) {
      ...
      verify(auditQueueService).sendPersonOnProbationAuditEvent(auditEvent)
      // test call to queue
    }
  }
  ...
  inner class traceParentHeader {
    ...
    internal fun `valid traceparent header and no operation id`() {
      ...
      verify(auditQueueService).sendPersonOnProbationAuditEvent(eventWithOperationId)
      // test call to queue
    }
