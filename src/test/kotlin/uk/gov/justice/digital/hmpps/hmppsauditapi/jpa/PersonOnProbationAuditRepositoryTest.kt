...
@DataJpaTest
class PersonOnProbationAuditRepositoryTest {
  ...
  inner class saveAuditEvents {
    ...
    internal fun `can write to repository with basic attributes`() {
      personOnProbationAuditRepository.save(
        PersonOnProbationAuditEvent(
          what = "An Event with basic attributes",
        ),
      )

      assertThat(personOnProbationAuditRepository.count()).isEqualTo(1)
      val auditEvent = personOnProbationAuditRepository.findAll().first()
  ...
  inner class filteredPageAuditEvents {
    ...
    internal fun `filter audit events by date range, service, what and who`() {
      ...
      assertThat(personOnProbationAuditRepository.count()).isEqualTo(3)
      val auditEvents = personOnProbationAuditRepository.findPage(
        Pageable.unpaged(),
